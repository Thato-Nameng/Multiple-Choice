...
namespace Student_Quiz_Management.Student_UserControl
{
    public partial class StudentExam : UserControl
    {
        ...
        private void LoadPracticeQuestions()
        {
            ...
            bool userResponseNotEmptyExam = CheckUserResponseNotEmpty(userName, userSurname, "Exam");

            if (userResponseNotEmptyExam)
            {
                DisableControls(); 
                MessageBox.Show("Exam is completed.", "Information",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                practiceQuestionsTable = fn.GetData($@"
                    SELECT Q.ID AS QuestionID, Q.Question, Q.OptionA, Q.OptionB, Q.OptionC, 
                        Q.OptionD, Q.Ans, Q.Subjects
                    FROM Questions Q
                        JOIN Registration R ON Q.Subjects = R.Subject
                    WHERE R.Name = '{userName}' 
                        AND R.Surname = '{userSurname}' 
                        AND Q.Test_Type = 'Exam'").Tables[0];
                ...
                {
                    DisableControls();
                    MessageBox.Show("There are no questions available for this exam.", "Information", 
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        private void SaveUserResponse(string selectedOption)
        {
            if (!string.IsNullOrEmpty(selectedOption))
            {
                ...
                {
                    string insertQuery = $@"
                        INSERT INTO UserResponse (RegistrationID, QuestionID, Name, Surname, 
                            Subject, Test_Type, Question_No, User_Answer, Ans) 
                        VALUES (
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                lblTime.Text = $"{timeRemaining / 60:D2}:{timeRemaining % 60:D2}"; 
            }
        }    
